...
# Mappatura swing personalizzata (verticale) per il tuo modello
SWING_VERTICAL_MAP = {
    "alto": 2,
    "medio_alto": 4,
    "medio_basso": 5,
...
SWING_REVERSE_MAP = {v: k for k, v in SWING_VERTICAL_MAP.items()}
...
class HonClimateEntity(CoordinatorEntity, ClimateEntity):
    ...
    def update_swing_mode(self, swing_horizontal, swing_vertical):
        try:
            vert = int(swing_vertical)
            if vert in SWING_REVERSE_MAP:
                self._attr_swing_mode = SWING_REVERSE_MAP[vert]
                return
        except Exception:
            pass
    ...
    async def async_set_swing_mode(self, swing_mode: str):
        parameters = {}
        if swing_mode in SWING_VERTICAL_MAP:
            parameters['windDirectionVertical'] = str(SWING_VERTICAL_MAP[swing_mode])
